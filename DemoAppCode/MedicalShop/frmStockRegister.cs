...
{
	public class frmStockRegister : Form
	{
		...
		public void CallDirectStock()
		{
			if (this.dgvRegister.CurrentRow != null)
			{
				...
				_frmStockEntry.DoWhenComingFromStockRegister(this, this.dgvRegister.CurrentRow.Cells[0].Value.ToString());
				base.Enabled = false;
			}
		}
		...
		{
			DataTable dataTable = this.StockMasterSP.StockMasterViewAllDateBetween(DateTime.Parse(this.dtpFromDate.Text), DateTime.Parse(this.dtpToDate.Text));
		...
		{
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(frmStockRegister));
			...
			this.label4.Location = new Point(184, 40);
