...
{
	public class rptManufacture : Form
	{
		...
		private void bwrk1_DoWork(object sender, DoWorkEventArgs e)
		{
			try
			{
				this.strHeading = "MANUFACTURE REPORT";
		...
		public void FillGridDetails()
		{
			...
			try
			{
				...
				dataTable = this.ManufacturerSP.ManufacturerViewAllExceptNA();
				if (dataTable.Rows.Count > 0)
				{
					foreach (DataRow row in dataTable.Rows)
					{
						if (row.ItemArray[1].ToString() != "NA")
						{
							...
							string str1 = row.ItemArray[2].ToString();
							...
							string str3 = row.ItemArray[5].ToString();
							...
							if (!(str4 == " , "))
							{
								...
								this.dgvReport[5, this.dgvReport.Rows.Count - 1].Value = str;
							}
							else
							{
								this.dgvReport[5, this.dgvReport.Rows.Count - 1].Value = "";
							}
						}
					}
		...
		private void InitializeComponent()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(rptManufacture));
		...
		public void LoadGridColumns()
		{
			try
			{
				...
				this.dgvReport[4, this.dgvReport.Rows.Count - 1].Value = "Email";
				this.dgvReport[5, this.dgvReport.Rows.Count - 1].Value = "Description";
