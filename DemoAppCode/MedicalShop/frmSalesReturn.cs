...
namespace MedicalShop
{
	public class frmSalesReturn : Form
	{
		...
		private frmsalesReturnRegister frmRegister;
		...
		private void btnRemove_Click(object sender, EventArgs e)
		{
			try
			{
				if (this.dgvSalesReturn.CurrentCell != null)
				{
					try
					{
						if (this.dgvSalesReturn.Rows.Count != 1)
						{
							this.dgvSalesReturn.Rows.RemoveAt(this.dgvSalesReturn.CurrentCell.RowIndex);
						}
						else
						{
							this.dgvSalesReturn.Rows.Clear();
						}
					}
					catch (Exception exception)
					{
					}
					if (this.dgvSalesReturn.Rows.Count > 0)
					{
						this.dgvSalesReturn.Focus();
						this.dgvSalesReturn.CurrentCell = this.dgvSalesReturn.Rows[0].Cells[0];
					}
					this.CalculateTotal();
				}
			}
		...
		private void btnSave_Click(object sender, EventArgs e)
		{
			...
			{
				DataTable dataTable = new DataTable();
				if (!((new SalesReturnMasterSP()).SalesReturnMasterViewAll().Rows.Count < 10 ? true : this.isInEditMode))
				{
					MessageBox.Show("Thank you for using Pharmacy.To gain access please buy full version", "Pharmacy", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
				}
				else if (!FinacialYearInfo._activeOrNot)
				{
		...
		public void CalculateTotal()
		{
			try
			{
				if (this.dgvSalesReturn.Rows.Count > 0)
				{
					...
					foreach (DataGridViewRow row in (IEnumerable)this.dgvSalesReturn.Rows)
					{
		...
		public void ClearFunction()
		{
			try
			{
				...
				this.txtVoucherNo.Text = this.GetMaxofSalesReturnMasterId();
			...
			try
			{
				...
				if ((e.KeyCode != Keys.Return ? false : this.cmbInvoice.Text != ""))
				{
					if (e.KeyCode == Keys.Return)
					{
						...
						{
							this.dgvSalesReturn.Focus();
							this.dgvSalesReturn.ClearSelection();
							this.dgvSalesReturn.CurrentCell = this.dgvSalesReturn.Rows[0].Cells[10];
							this.dgvSalesReturn.Rows[0].Cells[10].Selected = true;
						}
					}
				}
			}
			...
			try
			{
				if (this.cmbInvoice.SelectedValue != null)
				{
					if (this.cmbInvoice.Text != "")
					{
						...
						SalesReturnDetailsSP salesReturnDetailsSP = new SalesReturnDetailsSP();
						...
						salesMasterInfo = salesMasterSP.SalesMasterView(this.cmbInvoice.SelectedValue.ToString());
						if (salesMasterInfo.SalesMasterId != null)
						{
							accountLedgerInfo = accountLedgerSP.AccountLedgerView(salesMasterInfo.LedgerId);
							...
							int productId = 0;
							dataTable = salesDetailsSP.SalesDetailsPendingForReturn(salesMasterInfo.SalesMasterId);
							if (dataTable.Rows.Count > 0)
							{
								foreach (DataRow row in dataTable.Rows)
								{
									...
									if (!flag1)
									{
										...
										{
											this.dgvSalesReturn.Rows.Add();
											productBatchInfo = productBatchSP.ProductBatchView(row.ItemArray[1].ToString());
											this.dgvSalesReturn.Rows[productId].Cells[0].Value = productBatchInfo.ProductId;
											productInfo = this.productsp.ProductView(productBatchInfo.ProductId);
											this.dgvSalesReturn.Rows[productId].Cells[1].Value = productInfo.ProductName;
											this.dgvSalesReturn.Rows[productId].Cells[2].Value = productBatchInfo.BatchName;
											if (!(productBatchInfo.ExpiryDate.ToString() == "01/01/1753"))
											{
												this.dgvSalesReturn.Rows[productId].Cells[3].Value = productBatchInfo.ExpiryDate;
											}
											else
											{
												this.dgvSalesReturn.Rows[productId].Cells[3].Value = "";
											}
											this.dgvSalesReturn.Rows[productId].Cells[5].Value = row.ItemArray[2].ToString();
											this.dgvSalesReturn.Rows[productId].Cells[4].Value = productBatchInfo.PurchaseRate;
											this.dgvSalesReturn.Rows[productId].Cells[6].Value = productBatchInfo.MRP;
											manufacturerInfo = manufacturerSP.ManufacturerView(productInfo.ManufactureId);
											this.dgvSalesReturn.Rows[productId].Cells[7].Value = manufacturerInfo.ManufactureName;
											if (this.isInEditMode)
											{
												dataTable1 = salesReturnDetailsSP.SalesReturnDetailsViewAllByMasterId(this.txtVoucherNo.Text);
												foreach (DataRow dataRow in dataTable1.Rows)
												{
													decimal num6 = decimal.Parse(row.ItemArray[7].ToString());
													decimal num7 = decimal.Parse(row.ItemArray[6].ToString());
													if (!(dataRow.ItemArray[2].ToString() == row.ItemArray[0].ToString()))
													{
														this.dgvSalesReturn.Rows[productId].Cells[8].Value = Math.Round(num7, 2);
														this.dgvSalesReturn.Rows[productId].Cells[9].Value = 0;
														this.dgvSalesReturn.Rows[productId].Cells[10].Value = 0;
													}
													else
													{
														...
														{
															this.dgvSalesReturn.Rows[productId].Cells[8].Value = Math.Round(num7 + num6, 2);
															this.dgvSalesReturn.Rows[productId].Cells[9].Value = 0;
															DataGridViewCell item = this.dgvSalesReturn.Rows[productId].Cells[10];
											...
											{
												this.dgvSalesReturn.Rows[productId].Cells[8].Value = row.ItemArray[6].ToString();
												this.dgvSalesReturn.Rows[productId].Cells[9].Value = 0;
												this.dgvSalesReturn.Rows[productId].Cells[10].Value = 0;
											}
											unitInfo = unitSP.UnitView(productInfo.UnitId);
											this.dgvSalesReturn.Rows[productId].Cells[11].Value = unitInfo.UnitName;
											if (this.dgvSalesReturn.Rows[productId].Cells[10].Value == null)
											{
												this.dgvSalesReturn.Rows[productId].Cells[10].Value = 0;
											}
											else if (this.dgvSalesReturn.Rows[productId].Cells[10].Value.ToString() == "")
											{
												this.dgvSalesReturn.Rows[productId].Cells[10].Value = 0;
											}
											try
											{
												decimal.Parse(this.dgvSalesReturn.Rows[productId].Cells[10].Value.ToString());
											}
											catch (Exception exception)
											{
												this.dgvSalesReturn.Rows[productId].Cells[10].Value = 0;
											...
											{
												this.dgvSalesReturn.Rows[productId].Cells[12].Value = 0;
											}
											else
											{
												num = decimal.Parse(row.ItemArray[2].ToString());
												num1 = decimal.Parse(this.dgvSalesReturn.Rows[productId].Cells[10].Value.ToString());
												num2 = num * num1;
												this.dgvSalesReturn.Rows[productId].Cells[12].Value = Math.Round(num2, 2);
											}
											this.dgvSalesReturn.Rows[productId].Cells[13].Value = row.ItemArray[3].ToString();
											if (!(num2 == new decimal(0) ? true : !(row.ItemArray[3].ToString() != "")))
											{
												...
												this.dgvSalesReturn.Rows[productId].Cells[14].Value = num4;
											}
											else if (num2 == new decimal(0))
											{
												this.dgvSalesReturn.Rows[productId].Cells[14].Value = 0;
											}
											else if (decimal.Parse(row.ItemArray[4].ToString()) == new decimal(0))
											{
												this.dgvSalesReturn.Rows[productId].Cells[14].Value = Math.Round(num2);
											}
											if ((num4 == new decimal(0) ? true : !(decimal.Parse(row.ItemArray[4].ToString()) != new decimal(0))))
											{
												this.dgvSalesReturn.Rows[productId].Cells[17].Value = 0;
											}
											else
											{
												flag = false;
												...
												if (str == "False")
												{
													this.dgvSalesReturn.Rows[productId].Cells[17].Value = Math.Round((num4 * num5) / (new decimal(100) + num5), 2);
													this.dgvSalesReturn.Rows[productId].Cells[18].Value = num4;
												}
												else if (!(str == "True"))
												{
													this.dgvSalesReturn.Rows[productId].Cells[18].Value = num4;
												...
												{
													this.dgvSalesReturn.Rows[productId].Cells[17].Value = Math.Round((num4 * num5) / new decimal(100), 2);
													this.dgvSalesReturn.Rows[productId].Cells[18].Value = num4 + decimal.Parse(this.dgvSalesReturn.Rows[productId].Cells[17].Value.ToString());
												}
											}
											if (flag)
											{
												...
												{
													this.dgvSalesReturn.Rows[productId].Cells[18].Value = 0;
												}
												else
												{
													this.dgvSalesReturn.Rows[productId].Cells[18].Value = num4;
											...
											if (!(row.ItemArray[5].ToString() == "True"))
											{
												this.dgvSalesReturn.Rows[productId].Cells[15].Value = "Included";
											}
											else
											{
												this.dgvSalesReturn.Rows[productId].Cells[15].Value = "Excluded";
											}
											this.dgvSalesReturn.Rows[productId].Cells[16].Value = row.ItemArray[4].ToString();
											this.dgvSalesReturn.Rows[productId].Cells[19].Value = row.ItemArray[0].ToString();
											productId++;
										}
									}
								}
			...
			try
			{
				...
				if ((this.btnClear.Focused || this.btnDelete.Focused || this.btnClose.Focused || this.isFormClose ? false : MDIMedicalShop.MDIObj.ActiveMdiChild == this))
				{
					...
					if (e.ColumnIndex == 10)
					{
						...
						if (this.dgvSalesReturn.Rows[e.RowIndex].Cells[10].Value == null)
						{
							num = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[5].Value.ToString());
							num2 = num * num1;
							this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value = Math.Round(num2, 2);
						}
						else if (!(this.dgvSalesReturn.Rows[e.RowIndex].Cells[10].Value.ToString() != ""))
						{
							num = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[5].Value.ToString());
							num2 = num * num1;
							this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value = Math.Round(num2, 2);
						...
						{
							num3 = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[8].Value.ToString());
							num = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[5].Value.ToString());
							num1 = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[10].Value.ToString());
							if (!(num1 <= num3))
							{
								MessageBox.Show(string.Concat("Only ", Math.Round(num3, 0), " remaining"), "Pharmacy", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[10].Value = 0;
							}
							else
							{
								num2 = num * num1;
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value = num2;
							}
						}
						if ((this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value == null ? true : this.dgvSalesReturn.Rows[e.RowIndex].Cells[13].Value == null))
						{
							if (this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value == null)
							{
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value = 0;
							}
							else if (this.dgvSalesReturn.Rows[e.RowIndex].Cells[13].Value == null)
							{
								if (!(this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value.ToString() != "0.00"))
								{
									this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value = 0;
								}
								else
								{
									this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value = this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value.ToString();
								}
							}
						}
						else if (!(!(this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value.ToString() != "0") || !(this.dgvSalesReturn.Rows[e.RowIndex].Cells[13].Value.ToString() != "") ? true : !(this.dgvSalesReturn.Rows[e.RowIndex].Cells[13].Value.ToString() != "")))
						{
							decimal num4 = new decimal(0, 0, 0, false, 2);
							decimal num5 = new decimal(0, 0, 0, false, 2);
							num4 = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value.ToString());
							num5 = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[13].Value.ToString()) / new decimal(100);
							if (!(num4 >= num5))
							{
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value = num4 - new decimal(0);
							}
							else
							{
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value = num4 - (num4 * num5);
							}
						}
						else if (decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value.ToString()) == new decimal(0))
						{
							this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value = 0;
						}
						else if (this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value.ToString() == "")
						{
							this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value = 0;
						}
						else if (this.dgvSalesReturn.Rows[e.RowIndex].Cells[13].Value.ToString() == "")
						{
							this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value = this.dgvSalesReturn.Rows[e.RowIndex].Cells[12].Value.ToString();
						}
						if ((this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value == null ? true : this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value == null))
						{
							this.dgvSalesReturn.Rows[e.RowIndex].Cells[17].Value = 0;
						}
						else if ((this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value.ToString() == "" ? true : !(this.dgvSalesReturn.Rows[e.RowIndex].Cells[15].Value.ToString() != "")))
						{
							this.dgvSalesReturn.Rows[e.RowIndex].Cells[17].Value = 0;
						}
						else
						{
							...
							if (this.dgvSalesReturn.Rows[e.RowIndex].Cells[16].Value != null)
							{
								num6 = (!(this.dgvSalesReturn.Rows[e.RowIndex].Cells[16].Value.ToString() == "") ? decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[16].Value.ToString()) : new decimal(0));
							}
							else
							{
								num6 = new decimal(0);
							}
							num7 = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value.ToString());
							str = this.dgvSalesReturn.Rows[e.RowIndex].Cells[15].Value.ToString();
							if (str == "Included")
							{
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[17].Value = Math.Round((num7 * num6) / (new decimal(100) + num6), 2);
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[18].Value = num7;
							}
							...
							{
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[17].Value = Math.Round((num7 * num6) / new decimal(100), 2);
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[18].Value = num7 + decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[17].Value.ToString());
							}
						}
						if (flag)
						{
							if (this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value == null)
							{
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[18].Value = 0;
							}
							else if (!(this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value.ToString() != ""))
							{
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[18].Value = 0;
							}
							else
							{
								this.dgvSalesReturn.Rows[e.RowIndex].Cells[18].Value = this.dgvSalesReturn.Rows[e.RowIndex].Cells[14].Value.ToString();
							}
						}
						this.CalculateTotal();
					}
					DataTable dataTable = new DataTable();
					if ((e.ColumnIndex == 1 ? true : e.ColumnIndex == 10))
					{
						if ((this.dgvSalesReturn.Rows[e.RowIndex].Cells[0].Value == null || this.dgvSalesReturn.Rows[e.RowIndex].Cells[2].Value == null ? false : this.dgvSalesReturn.Rows[e.RowIndex].Cells[10].Value != null))
						{
							if ((!(this.dgvSalesReturn.Rows[e.RowIndex].Cells[0].Value.ToString() != "") || !(this.dgvSalesReturn.Rows[e.RowIndex].Cells[2].Value.ToString() != "") ? false : this.dgvSalesReturn.Rows[e.RowIndex].Cells[10].Value.ToString() != ""))
							{
								if (decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[10].Value.ToString()) > new decimal(0))
								{
									string str1 = this.dgvSalesReturn.Rows[e.RowIndex].Cells[0].Value.ToString();
									dataTable = this.productsp.GetCurrentStockOfProduct(str1);
									ProductInfo productInfo = new ProductInfo();
									...
									if (productInfo.StockMaximumLevel > new decimal(0))
									{
										...
										decimal returnedQty = new decimal(0);
										if (dataTable.Rows.Count > 0)
										{
											num8 = decimal.Parse(dataTable.Rows[0].ItemArray[0].ToString());
											if (this.isInEditMode)
											{
												SalesReturnDetailsSP salesReturnDetailsSP = new SalesReturnDetailsSP();
												SalesReturnDetailsInfo salesReturnDetailsInfo = new SalesReturnDetailsInfo();
												salesReturnDetailsInfo = salesReturnDetailsSP.SalesReturnDetailsView(this.dgvSalesReturn.Rows[e.RowIndex].Cells[19].Value.ToString());
												returnedQty = salesReturnDetailsInfo.ReturnedQty;
												num10 = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[10].Value.ToString());
												num9 = (num8 - returnedQty) + num10;
												if (productInfo.StockMaximumLevel < num9)
												{
											...
											{
												num9 = decimal.Parse(this.dgvSalesReturn.Rows[e.RowIndex].Cells[10].Value.ToString());
												num10 = num8 + num9;
												if (productInfo.StockMaximumLevel < num10)
												{
													MessageBox.Show("Exceeded maximum stock level", "Pharmacy", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
												}
											}
		...
		private void dgvPurchaseInvoice_CellLeave(object sender, DataGridViewCellEventArgs e)
		{
			...
			try
			{
				...
				{
					flag = (this.dgvSalesReturn.CurrentCell.ColumnIndex == 4 || this.dgvSalesReturn.CurrentCell.ColumnIndex == 5 || this.dgvSalesReturn.CurrentCell.ColumnIndex == 6 || this.dgvSalesReturn.CurrentCell.ColumnIndex == 8 || this.dgvSalesReturn.CurrentCell.ColumnIndex == 9 || this.dgvSalesReturn.CurrentCell.ColumnIndex == 10 || this.dgvSalesReturn.CurrentCell.ColumnIndex == 12 || this.dgvSalesReturn.CurrentCell.ColumnIndex == 13 || this.dgvSalesReturn.CurrentCell.ColumnIndex == 14 ? false : this.dgvSalesReturn.CurrentCell.ColumnIndex != 16);
				}
				if (!flag)
				{
					try
					{
						decimal.Parse(this.dgvSalesReturn.CurrentCell.Value.ToString());
					}
					catch (Exception exception)
					{
						this.dgvSalesReturn.CurrentCell.Value = 0;
					}
				}
			}
			catch (Exception exception2)
		...
		private void dgvPurchaseReturn_KeyPress(object sender, KeyPressEventArgs e)
		{
			try
			{
				if (e.KeyChar == '\r')
				{
					if (this.dgvSalesReturn.CurrentCell == this.dgvSalesReturn[18, this.dgvSalesReturn.Rows.Count - 1])
					{
						this.txtDescription.Focus();
						this.dgvSalesReturn.ClearSelection();
					}
		...
		private void dgvPurchaseReturn_KeyUp(object sender, KeyEventArgs e)
		{
			try
			{
				if (e.KeyCode == Keys.Back)
				{
					if ((this.dgvSalesReturn.CurrentCell == null || this.dgvSalesReturn.CurrentCell.ColumnIndex != 0 ? false : this.dgvSalesReturn.CurrentCell.RowIndex == 0))
					{
						if (this.p != 1)
						{
							frmSalesReturn _frmSalesReturn = this;
							_frmSalesReturn.p = _frmSalesReturn.p + 1;
						}
						else
						{
							this.p = 0;
		...
		private void dgvSalesReturn_CellEndEdit()
		{
		}
		...
		public void DoWhenComingFromRegister(frmsalesReturnRegister frmsalesreturnregister, string strsalesReturnMasterId)
		{
			try
			{
				base.Show();
				...
				this.frmRegister = frmsalesreturnregister;
				...
				SalesReturnDetailsSP salesReturnDetailsSP = new SalesReturnDetailsSP();
				SalesReturnDetailsInfo salesReturnDetailsInfo = new SalesReturnDetailsInfo();
				salesReturnMasterInfo = salesReturnMasterSP.SalesReturnMasterView(strsalesReturnMasterId);
				if (salesReturnMasterInfo.SalesReturnMasterId != null)
				{
					this.txtVoucherNo.Text = salesReturnMasterInfo.SalesReturnMasterId;
					this.dtpDate.Text = salesReturnMasterInfo.Date.ToString();
					this.FillComboInvoice();
					this.cmbInvoice.SelectedValue = salesReturnMasterInfo.SalesMasterId;
				}
			}
			catch (Exception exception1)
			{
		...
		public void FillComboInvoice()
		{
			try
			{
				SalesMasterSP salesMasterSP = new SalesMasterSP();
				DataTable dataTable = new DataTable();
				dataTable = salesMasterSP.SalesMasterForReturn();
				if (dataTable.Rows.Count > 0)
				{
					this.cmbInvoice.DataSource = dataTable;
					this.cmbInvoice.DisplayMember = "SalesInvoiceNo";
					this.cmbInvoice.ValueMember = "SalesMasterId";
				}
			}
			catch (Exception exception1)
			{
		...
		public string GetMaxofSalesReturnMasterId()
		{
			string str = "";
			try
			{
				str = (new SalesReturnMasterSP()).SalesReturnMasterGetMax().ToString();
			}
			catch (Exception exception1)
			{
				Exception exception = exception1;
		...
		{
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(frmSalesReturn));
			...
			((ISupportInitialize)this.dgvSalesReturn).BeginInit();
			base.SuspendLayout();
			...
			this.panel8.Controls.Add(this.dgvSalesReturn);
			...
			this.dgvSalesReturn.BackgroundColor = Color.White;
			this.dgvSalesReturn.ColumnHeadersHeightSizeMode = DataGridViewColumnHeadersHeightSizeMode.DisableResizing;
			DataGridViewColumnCollection columns = this.dgvSalesReturn.Columns;
			...
			this.dgvSalesReturn.GridColor = Color.WhiteSmoke;
			this.dgvSalesReturn.Location = new Point(12, 140);
			this.dgvSalesReturn.Name = "dgvSalesReturn";
			this.dgvSalesReturn.RowHeadersVisible = false;
			this.dgvSalesReturn.RowHeadersWidthSizeMode = DataGridViewRowHeadersWidthSizeMode.DisableResizing;
			this.dgvSalesReturn.Size = new System.Drawing.Size(754, 149);
			this.dgvSalesReturn.TabIndex = 7;
			this.dgvSalesReturn.CellLeave += new DataGridViewCellEventHandler(this.dgvPurchaseInvoice_CellLeave);
			this.dgvSalesReturn.KeyDown += new KeyEventHandler(this.dgvPurchaseInvoice_KeyDown);
			this.dgvSalesReturn.Leave += new EventHandler(this.dgvPurchaseInvoice_Leave);
			this.dgvSalesReturn.CellEndEdit += new DataGridViewCellEventHandler(this.dgvPurchaseInvoice_CellEndEdit);
			this.dgvSalesReturn.EditingControlShowing += new DataGridViewEditingControlShowingEventHandler(this.dgvPurchaseInvoice_EditingControlShowing);
			this.dgvSalesReturn.KeyUp += new KeyEventHandler(this.dgvPurchaseReturn_KeyUp);
			this.dgvSalesReturn.KeyPress += new KeyPressEventHandler(this.dgvPurchaseReturn_KeyPress);
			this.dgvSalesReturn.CellContentClick += new DataGridViewCellEventHandler(this.dgvPurchaseReturn_CellContentClick);
			...
			this.Column9.HeaderText = "Sale Qty";
			...
			((ISupportInitialize)this.dgvSalesReturn).EndInit();
			...
			try
			{
				...
				else
				{
					...
					else
					{
						...
						this.dgvSalesReturn.ClearSelection();
						this.dgvSalesReturn.CurrentCell = this.dgvSalesReturn.Rows[0].Cells[0];
						this.dgvSalesReturn.Rows[0].Cells[0].Selected = true;
					}
				}
			}
			catch (Exception exception1)
			...
			try
			{
				...
				if (this.cmbInvoice.SelectedValue != null)
				{
					if (this.cmbInvoice.Text != "")
					{
						...
						if (this.isInEditMode)
						{
							DataTable dataTable = this.ledgerpostingsp.LedgerPostingViewByVoucherTypeAndVoucherNumber(this.txtVoucherNo.Text, "Sales Return");
							if (dataTable.Rows.Count > 0)
							{
								ledgerPostingInfo.LedgerPostingId = dataTable.Rows[0][0].ToString();
								salesMasterInfo = salesMasterSP.SalesMasterView(this.cmbInvoice.SelectedValue.ToString());
								ledgerPostingInfo.LedgerId = salesMasterInfo.LedgerId;
		...
		public void SaveToSalesReturnMaster()
		{
			try
			{
				string text = "";
				...
				if ((this.txtVoucherNo.Text == "" ? false : this.cmbInvoice.SelectedValue != null))
				{
					...
					{
						text = this.GetMaxofSalesReturnMasterId();
					...
					salesReturnMasterInfo.SalesReturnMasterId = text;
					salesReturnMasterInfo.Date = DateTime.Parse(this.dtpDate.Text);
					salesReturnMasterInfo.SalesMasterId = this.cmbInvoice.SelectedValue.ToString();
					salesReturnMasterInfo.Description = this.txtDescription.Text.Trim();
					if (this.isInEditMode)
					{
						salesReturnDetailsSP.SalesReturnDetailsDeleteByMasterId(this.txtVoucherNo.Text);
					}
					foreach (DataGridViewRow row in (IEnumerable)this.dgvSalesReturn.Rows)
					{
						if ((row.Cells[0].Value == null || row.Cells[1].Value == null ? false : row.Cells[10].Value != null))
						{
							if ((!(row.Cells[0].Value.ToString() != "") || !(row.Cells[1].Value.ToString() != "") ? false : row.Cells[10].Value.ToString() != ""))
							{
								if (decimal.Parse(row.Cells[10].Value.ToString()) > new decimal(0))
								{
									...
									salesReturnDetailsInfo.SalesReturnMasterId = text;
									salesReturnDetailsInfo.SalesDetailsId = row.Cells[19].Value.ToString();
									salesReturnDetailsInfo.ReturnedQty = decimal.Parse(row.Cells[10].Value.ToString());
									salesReturnDetailsInfo.ReturnedFreeQty = decimal.Parse(row.Cells[18].Value.ToString());
									salesReturnDetailsInfo.Description = "";
									salesReturnDetailsSP.SalesReturnDetailsAdd(salesReturnDetailsInfo);
								}
							}
						}
					}
					if (this.isAnyReturn)
					{
						if (this.isInEditMode)
						{
							salesReturnMasterSP.SalesReturnMasterEdit(salesReturnMasterInfo);
						}
						else
						{
							salesReturnMasterSP.SalesReturnMasterAdd(salesReturnMasterInfo);
						}
					}
				}
			}
		...
		public void SaveToStockPosting()
		{
			try
			{
				...
				if (this.txtVoucherNo.Text != "")
				{
					if (this.isInEditMode)
					{
						stockPostingSP.StockPostingDeleteByVoucherAndType(this.txtVoucherNo.Text, "Sales Return");
					}
					dataTable = salesReturnDetailsSP.SalesReturnDetailsViewAllByMasterId(this.txtVoucherNo.Text);
					foreach (DataRow row in dataTable.Rows)
					{
						stockPostingInfo.VoucherNumber = row.ItemArray[1].ToString();
						salesDetailsInfo = salesDetailsSP.SalesDetailsView(row.ItemArray[2].ToString());
						stockPostingInfo.ProductBatchId = salesDetailsInfo.ProductBatchId;
						stockPostingInfo.InwardQuantity = decimal.Parse(row.ItemArray[3].ToString());
						stockPostingInfo.OutwardQuantity = new decimal(0);
						if (!salesDetailsInfo.DirectSaleOrNot)
						{
							stockPostingInfo.VoucherType = "Sales Return";
						}
						else
						{
		...
		private void TextBoxCellEditControlKeyPress(object sender, KeyPressEventArgs e)
		{
			try
			{
				if (this.dgvSalesReturn.CurrentCell.ColumnIndex == 10)
				{
			...
			try
			{
				if (e.KeyCode == Keys.Back)
				{
					if ((this.txtDescription.Text == "" ? true : this.txtDescription.SelectionStart == 0))
					{
						...
						{
							this.dgvSalesReturn.CurrentCell = this.dgvSalesReturn.Rows[this.dgvSalesReturn.Rows.Count - 1].Cells[18];
							this.dgvSalesReturn.CurrentCell.Selected = true;
							this.dgvSalesReturn.Focus();
						}
					}
				}
		...
		private void txtDescription_KeyPress(object sender, KeyPressEventArgs e)
		{
			try
			{
				...
				{
					frmSalesReturn _frmSalesReturn = this;
					_frmSalesReturn.inDescriptionCount = _frmSalesReturn.inDescriptionCount + 1;
		...
		private void txtSearchFor_TextChanged(object sender, EventArgs e)
		{
			...
			{
				string str = "";
				for (int i = 0; i < this.dgvSalesReturn.Rows.Count; i++)
				{
					if (this.dgvSalesReturn.Rows[i].Cells[0].Value != null)
					{
						str = (!(this.cmbSearchBy.Text == "Product Code") ? this.dgvSalesReturn[1, i].Value.ToString().ToLower() : this.dgvSalesReturn[0, i].Value.ToString().ToLower());
						if (!str.StartsWith(this.txtSearchFor.Text.ToLower()))
						{
							this.dgvSalesReturn.Rows[i].Visible = false;
						}
						else
						{
							this.dgvSalesReturn.Rows[i].Visible = true;
						}
					}
				}
			}
