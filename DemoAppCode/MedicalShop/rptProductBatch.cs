...
{
	public class rptProductBatch : Form
	{
		...
		private void bwrk1_DoWork(object sender, DoWorkEventArgs e)
		{
			try
			{
				this.strHeading = "PRODUCT BATCH REPORT";
		...
		public void FillBatchGrid()
		{
			try
			{
				...
				foreach (DataRow row in dataTable.Rows)
				{
					...
					DataGridViewCell item = this.dgvReport.Rows[str].Cells[7];
					DateTime dateTime = DateTime.Parse(row.ItemArray[7].ToString());
					item.Value = dateTime.ToString("dd-MMM-yyyy");
					if (this.dgvReport.Rows[str].Cells[3].Value.ToString().Trim().ToLower() == "na")
					{
						this.dgvReport.Rows[str].Cells[7].Value = "";
					}
					this.dgvReport.Rows[str].Cells[8].Value = row.ItemArray[6].ToString();
					str++;
				}
			}
			catch (Exception exception1)
		...
		private void InitializeComponent()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(rptProductBatch));
			...
			this.groupBox4.Location = new Point(752, 116);
			...
			this.label5.Text = "Product Batch";
			...
			this.groupBox2.Location = new Point(15, 169);
			...
			this.cmbSearchFor.Location = new Point(265, 15);
			...
			this.cmbSearchBy.Location = new Point(80, 15);
		...
		public void LoadGridColumns()
		{
			try
			{
				...
				this.dgvReport[7, this.dgvReport.Rows.Count - 1].Value = "Expiry Date";
				this.dgvReport[8, this.dgvReport.Rows.Count - 1].Value = "Group";
