...
namespace MedicalShop
{
	public class rptCashBook : Form
	{
		...
		public void FillGrid()
		{
			...
			dataTable = accountLedgerSP.CashBook(DateTime.Parse(this.dtpFrom.Text), DateTime.Parse(this.dtpTo.Text), flag);
			int str = 1;
			...
			foreach (DataGridViewRow dataGridViewRow in (IEnumerable)this.dgvReport.Rows)
			{
				if ((dataGridViewRow.Index <= 0 ? false : dataGridViewRow.Visible))
				{
					...
					try
					{
						num2 = decimal.Parse(dataGridViewRow.Cells[3].Value.ToString());
					}
					...
					try
					{
						num3 = decimal.Parse(dataGridViewRow.Cells[4].Value.ToString());
					}
					catch (Exception exception1)
					{
					}
			...
			this.dgvReport.Rows[this.dgvReport.Rows.Count - 1].Cells[3].Value = num1;
			this.dgvReport.Rows[this.dgvReport.Rows.Count - 1].Cells[4].Value = num;
			if (num1 != num)
			{
				this.dgvReport.Rows.Add();
				...
				{
					num4 = num - num1;
					this.dgvReport.Rows[this.dgvReport.Rows.Count - 1].Cells[2].Value = string.Concat("Balance : ", num4, "Cr");
				}
				else
				{
					num4 = num1 - num;
					this.dgvReport.Rows[this.dgvReport.Rows.Count - 1].Cells[2].Value = string.Concat("Balance : ", num4, "Dr");
				}
		...
		private void FillSerachCombo()
		{
			...
			else
			{
				...
				dataTable = new DataTable();
				dataTable = accountLedgerSP1.AccountGroupViewAllUnderCash();
				this.cmbSearchFor.DataSource = dataTable;
		...
		private void InitializeComponent()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(rptCashBook));
			...
			this.groupBox4.Location = new Point(757, 130);
