...
namespace MedicalShop
{
	public class rptBankBook : Form
	{
		...
		public void FillGrid()
		{
			...
			dataTable = accountLedgerSP.BankBook(DateTime.Parse(this.dtpFrom.Text), DateTime.Parse(this.dtpTo.Text), flag);
			int str = 1;
			...
			foreach (DataRow row in dataTable.Rows)
			{
				...
				this.dgvReport.Rows[str].Cells["Account Ledger"].Value = row.ItemArray[0].ToString();
				this.dgvReport.Rows[str].Cells["Account Group"].Value = row.ItemArray[1].ToString();
			...
			foreach (DataGridViewRow dataGridViewRow in (IEnumerable)this.dgvReport.Rows)
			{
				if ((dataGridViewRow.Index <= 0 ? false : dataGridViewRow.Visible))
				{
					...
					try
					{
						num3 = decimal.Parse(dataGridViewRow.Cells[3].Value.ToString());
					}
					...
					try
					{
						num4 = decimal.Parse(dataGridViewRow.Cells[4].Value.ToString());
					}
					catch (Exception exception1)
					{
					}
			...
			this.dgvReport.Rows[this.dgvReport.Rows.Count - 1].Cells[3].Value = num2;
			this.dgvReport.Rows[this.dgvReport.Rows.Count - 1].Cells[4].Value = num1;
			if (num2 != num1)
			{
				this.dgvReport.Rows.Add();
				if (!(num2 > num1))
				{
					num = num1 - num2;
					this.dgvReport.Rows[this.dgvReport.Rows.Count - 1].Cells[2].Value = string.Concat("Balance : ", num, "Cr");
				}
				else
				{
					num = num2 - num1;
					this.dgvReport.Rows[this.dgvReport.Rows.Count - 1].Cells[2].Value = string.Concat("Balance : ", num, "Dr");
				}
		...
		private void FillSerachCombo()
		{
			...
			if (this.cmbSearchBy.Text == "Account Ledger")
			{
				...
				this.cmbSearchFor.DisplayMember = "Account Ledger Name";
				this.cmbSearchFor.ValueMember = "Account Ledger Id";
				this.cmbSearchFor.SelectedValue = "2";
			}
			else if (!(this.cmbSearchBy.Text == "Account Group"))
			{
				this.cmbSearchFor.SelectedIndex = -1;
			}
			else
			{
				AccountLedgerSP accountLedgerSP1 = new AccountLedgerSP();
				dataTable = new DataTable();
				dataTable = accountLedgerSP1.AccountGroupViewAllUnderBank();
				this.cmbSearchFor.DataSource = dataTable;
		...
		private void InitializeComponent()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(rptBankBook));
			...
			this.groupBox4.Location = new Point(760, 130);
		...
		public void LoadGridColumns()
		{
			try
			{
				...
				this.dgvReport[1, this.dgvReport.Rows.Count - 1].Value = "Account Ledger";
				this.dgvReport[2, this.dgvReport.Rows.Count - 1].Value = "Account Group";
		...
		private void rbtnGroupWise_CheckedChanged(object sender, EventArgs e)
		{
			try
			{
				...
				dataRow[dataColumn] = "All";
				dataTable.Rows.Add(dataRow);
				dataRow = dataTable.NewRow();
				dataRow[dataColumn] = "Account Group";
				dataTable.Rows.Add(dataRow);
				if (this.rbtnLedgerWise.Checked)
				{
					dataRow = dataTable.NewRow();
					dataRow[dataColumn] = "Account Ledger";
					dataTable.Rows.Add(dataRow);
				}
				this.cmbSearchBy.DataSource = dataTable;
				this.cmbSearchBy.DisplayMember = dataTable.Columns[0].ToString();
