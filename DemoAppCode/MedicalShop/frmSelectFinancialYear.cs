...
{
	public class frmSelectFinancialYear : Form
	{
		...
		public void FillFinacialYearGrid()
		{
			...
			foreach (DataRow row in dataTable.Rows)
			{
				...
				this.dgvFinancial.Rows[str].Cells[0].Value = row.ItemArray[0].ToString();
				this.dgvFinancial.Rows[str].Cells[2].Value = row.ItemArray[1].ToString();
				this.dgvFinancial.Rows[str].Cells[3].Value = row.ItemArray[2].ToString();
				this.dgvFinancial.Rows[str].Cells[4].Value = row.ItemArray[3].ToString();
				DataGridViewCell item = this.dgvFinancial.Rows[str].Cells[1];
				date = DateTime.Parse(row.ItemArray[1].ToString());
				string str1 = date.ToString("dd-MMM-yyyy");
				date = DateTime.Parse(row.ItemArray[2].ToString());
				item.Value = string.Concat(str1, " - ", date.ToString("dd-MMM-yyyy"));
				str++;
			}
		}
		...
		private void InitializeComponent()
		{
			...
			DataGridViewColumn[] d = new DataGridViewColumn[] { this.ID, this.FinacialYear, this.From, this.To, this.Status };
			columns.AddRange(d);
			...
			this.FinacialYear.HeaderText = "Finacial Year";
			...
			base.Name = "frmSelectFinancialYear";
		...
		public void SetFinacialYear()
		{
			...
			if (this.dgvFinancial.CurrentRow != null)
			{
				if ((this.dgvFinancial.CurrentRow.Cells[0].Value == null ? false : this.dgvFinancial.CurrentRow.Cells[0].Value.ToString().Trim() != ""))
				{
					...
					{
						if ((DateTime.Parse(this.dgvFinancial.CurrentRow.Cells[2].Value.ToString()) != FinacialYearInfo._fromDate ? false : !(DateTime.Parse(this.dgvFinancial.CurrentRow.Cells[3].Value.ToString()) != FinacialYearInfo._toDate)))
						{
							flag = true;
						}
