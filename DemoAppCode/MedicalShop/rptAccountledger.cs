...
{
	public class rptAccountledger : Form
	{
		...
		private void btnClear_Click(object sender, EventArgs e)
		{
			try
			{
				this.rptAccountledger_Load(sender, e);
			}
		...
		private void bwrk1_DoWork(object sender, DoWorkEventArgs e)
		{
			try
			{
				this.strHeading = "ACCOUNT LEDGER REPORT";
		...
		public void FillAccountLedgerGrid()
		{
			try
			{
				...
				foreach (DataRow row in dataTable.Rows)
				{
					...
					this.dgvReport[0, this.dgvReport.Rows.Count - 1].Value = this.dgvReport.RowCount - 1;
					this.dgvReport[1, this.dgvReport.Rows.Count - 1].Value = row.ItemArray[1].ToString();
					this.AccountGroupInfo = this.accountgroupSP.AccountGroupView(row.ItemArray[2].ToString());
					this.dgvReport[2, this.dgvReport.Rows.Count - 1].Value = this.AccountGroupInfo.AccountGroupName;
					if (row.ItemArray[4].ToString() == true.ToString())
					{
						this.dgvReport[3, this.dgvReport.Rows.Count - 1].Value = string.Concat(row.ItemArray[3].ToString(), "  Dr");
					}
					else if (row.ItemArray[4].ToString() == false.ToString())
					{
						this.dgvReport[3, this.dgvReport.Rows.Count - 1].Value = string.Concat(row.ItemArray[3].ToString(), "  Cr");
					}
					string str = row.ItemArray[5].ToString();
					...
					if (!(str1 == " , "))
					{
						...
						this.dgvReport[4, this.dgvReport.Rows.Count - 1].Value = str2;
					}
					else
					{
						this.dgvReport[4, this.dgvReport.Rows.Count - 1].Value = "";
					}
					num++;
				}
				this.dgvReport.Columns[1].SortMode = DataGridViewColumnSortMode.NotSortable;
		...
		private void InitializeComponent()
		{
			...
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(rptAccountledger));
			...
			this.groupBox4.Location = new Point(752, 149);
			...
			this.groupBox1.Controls.Add(this.rbtnNonBuiltIn);
			this.groupBox1.Controls.Add(this.rbtnBuiltIn);
			...
			this.rbtnAll.Location = new Point(9, 19);
			this.rbtnAll.Name = "rbtnAll";
			this.rbtnAll.Size = new System.Drawing.Size(36, 17);
			this.rbtnAll.TabIndex = 456466;
			...
			this.cmbSearchBy.TabIndex = 456462;
			...
			base.Name = "rptAccountledger";
			base.StartPosition = FormStartPosition.CenterScreen;
			base.WindowState = FormWindowState.Maximized;
			base.KeyPress += new KeyPressEventHandler(this.rptAccountledger_KeyPress);
			base.Load += new EventHandler(this.rptAccountledger_Load);
		...
		public void LoadGridColumns()
		{
			try
			{
				...
				this.dgvReport.Columns.Add("Ledger Name", "Ledger Name");
				this.dgvReport.Columns.Add("Group", "Group");
				this.dgvReport.Columns.Add("Opening Balance", "Group Name");
				this.dgvReport.Columns.Add("Description", "Description");
				this.dgvReport.Columns[0].Width = 50;
				this.dgvReport.Columns[1].Width = 300;
				this.dgvReport.Rows.Add(1);
				this.dgvReport.Rows[0].Frozen = true;
				this.dgvReport[0, this.dgvReport.Rows.Count - 1].Value = "Sl No";
				this.dgvReport[1, this.dgvReport.Rows.Count - 1].Value = "Ledger Name";
				this.dgvReport[2, this.dgvReport.Rows.Count - 1].Value = "Group";
				this.dgvReport[3, this.dgvReport.Rows.Count - 1].Value = "Opening Balance";
				this.dgvReport[4, this.dgvReport.Rows.Count - 1].Value = "Description";
		...
		private void rptAccountledger_KeyPress(object sender, KeyPressEventArgs e)
		{
			try
			{
				if (e.KeyChar == '\u001B')
		...
		private void rptAccountledger_Load(object sender, EventArgs e)
		{
			try
			{
