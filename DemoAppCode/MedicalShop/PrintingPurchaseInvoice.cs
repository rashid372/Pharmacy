...
namespace MedicalShop
{
    ...
    {
        public PrintingPurchaseInvoice()
        {
        ...
        private Font InvTitleFont = new Font("Times New Roman", 30, FontStyle.Bold);
        private Font InvFooterFont = new Font("Times New Roman", 10, FontStyle.Regular);
        // Title Font height
        private int InvTitleHeight;
        // SubTitle Font
        private Font InvSubTitleFont = new Font("Times New Roman", 14, FontStyle.Regular);
        ...
        private string GetReciptBody()
        {
            ...
            body.AppendLine("NAME           PRICE     QTY    TOTAL ");
            body.AppendLine("--------------------------------------------");
            if (invoicedatainfo.invoiceitem.Count > 0)
            {
                foreach (var item in invoicedatainfo.invoiceitem)
                {
                    ...
                    {
                        body.AppendLine(FormateString(item.ProductName.Remove(13), 14) + " " + FormateString(item.Rate.ToString(), 10) + " " + FormateString(item.Quantity.ToString(), 5) + " " + FormateString(item.TotalAmount.ToString(), 10));
                    }
                    else
                    {
                        body.AppendLine(FormateString(item.ProductName, 14) + " " + FormateString(item.Rate.ToString(), 10) + " " + FormateString(item.Quantity.ToString(), 5) + " " + FormateString(item.TotalAmount.ToString(), 10));
                    }
            ...
            body.AppendLine("--------------------------------------------");
            body.AppendLine("Thank you for visiting at Mumtaz Medicose.");
        ...
        {// Set Invoice Table:
            ...
            foreach (var item in invoicedatainfo.invoiceitem.OrderBy(x => x.ProductName))
            {
                ...
                //invoicedatainfo.invoiceitem.Remove(item);
        ...
        {// Set Invoice Total:
         // Draw line:
            ...
            int xFreight = xRightEdg - (int)g.MeasureString(FreightValue, InvoiceFont).Width + 20;
            ...
            // Write Invoice Total:
            //int xWarranty = AmountPosition - (int)g.MeasureString("Warranty", InvFooterFont).Width;
        ...
        public void PrintReport()
        {
            try
            {
                ...
                if (PrinterInfo.ActivePrinterName.ToUpper().Contains("HP") || PrinterInfo.ActivePrinterName.ToUpper().Contains("CUTE"))
                {
                    prnDocument.Print();
                }
                else
