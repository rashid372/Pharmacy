...
{
	public class rptTrialBalance : Form
	{
		...
		public void FillGrid()
		{
			...
			dataTable = accountLedgerSP.TrialBalance(DateTime.Parse(this.dtpFrom.Text), DateTime.Parse(this.dtpTo.Text), flag);
			int str = 1;
		...
		private void FillGridAccordingToSearch()
		{
			...
			this.FillGrid();
			int index = 0;
			index = this.dgvReport.Columns["Account Ledger"].Index;
			if (this.cmbSearchFor.Text != "All")
			{
				for (int i = 1; i < this.dgvReport.Rows.Count - 3; i++)
				{
				...
				this.dgvReport.Rows.RemoveAt(this.dgvReport.Rows.Count - 1);
				this.dgvReport.Rows.RemoveAt(this.dgvReport.Rows.Count - 1);
				this.dgvReport.Rows.RemoveAt(this.dgvReport.Rows.Count - 1);
				decimal num1 = new decimal(0);
				decimal num2 = new decimal(0);
				foreach (DataGridViewRow row in (IEnumerable)this.dgvReport.Rows)
				{
					if ((row.Index <= 0 ? false : row.Visible))
					{
						...
						try
						{
							num4 = decimal.Parse(row.Cells[4].Value.ToString());
						}
						catch (Exception exception1)
						{
						}
		...
		private void InitializeComponent()
		{
			...
			this.dgvReport.CellContentClick += new DataGridViewCellEventHandler(this.dgvReport_CellContentClick);
			...
			this.cmbSearchFor.Location = new Point(86, 13);
		...
		private void rbtnGroupWise_CheckedChanged(object sender, EventArgs e)
		{
			...
			else if (this.rbtnLedgerWise.Checked)
			{
