...
{
	public class frmGenericNames : Form
	{
		...
		private void btnDelete_Click_1(object sender, EventArgs e)
		{
			try
			{
                ...
                GenericNameSP.GenericNameDelete(this._ID);
			...
			try
			{
				if (this.dgvSearch.CurrentRow != null)
				{
					if (this.dgvSearch.Rows.Count > 0)
					{
						...
						this._GenericName = this.dgvSearch.CurrentRow.Cells[1].Value.ToString().ToLower();
						this.txtname.Text = this.dgvSearch.CurrentRow.Cells[1].Value.ToString();
		...
		public void dgdSearchFill()
		{
			try
			{
				...
				foreach (DataRow dataRow in dataTable.Rows)
				{
					...
					this.dgvSearch.Rows[str].Cells[2].Value = dataRow.ItemArray[2].ToString();
					str++;
				}
		...
		private void frmGenericNames_FormClosing(object sender, FormClosingEventArgs e)
		{
			try
			{
		...
		private void frmGenericNames_Load(object sender, EventArgs e)
		{
			try
			{
				this.dgdSearchFill();
		...
		{
			ComponentResourceManager componentResourceManager = new ComponentResourceManager(typeof(frmGenericNames));
			...
			this.txtSearch.Location = new Point(67, 10);
			...
			this.txtSearch.TextChanged += new EventHandler(this.txtsearch_TextChanged_1);
			this.label6.AutoSize = true;
			this.label6.Location = new Point(4, 15);
			...
			this.txtname.Location = new Point(134, 10);
			...
			this.label1.Text = "Generic Names";
			...
			this.dataGridViewTextBoxColumn2.HeaderText = "Generic Name ";
			...
			base.Name = "frmGenericNames";
			base.Padding = new System.Windows.Forms.Padding(7);
			base.StartPosition = FormStartPosition.CenterScreen;
			this.Text = "Generic Names";
			base.FormClosing += new FormClosingEventHandler(this.frmGenericNames_FormClosing);
			base.Load += new EventHandler(this.frmGenericNames_Load);
		...
		private void saveOrEditDetails()
		{
			try
			{
				...
				else
				{
					...
					string str = "";
					if (!(this.txtname.Text == "" ? true : !(this.btnSave.Text == "&Save")))
					{
						str = this.GenericNameSP.CheckGenericNamekReference(this.txtname.Text.Trim());
						if (!(str != "true"))
						{
							MessageBox.Show("Generic name already exist", "Pharmacy", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
							this.txtname.Focus();
							this.txtname.SelectAll();
						}
						else
						{
							this.GenericNameInfo.GenericName = this.txtname.Text.Trim();
							this.GenericNameInfo.Description = this.txtDescription.Text.Trim();
							this.strGroupIdForOtherForms = this.GenericNameSP.GenericNameAdd(this.GenericNameInfo);
					...
					else if (!(this.txtname.Text == "" ? true : !(this.btnSave.Text == "&Update")))
					{
						if (this.txtname.Text.Trim() != this._GenericName)
						{
							str = this.GenericNameSP.CheckGenericNamekReference(this.txtname.Text.Trim());
						}
						if (!(str != "true"))
						...
						{
							this.GenericNameInfo.GenericNameId = this._ID;
							this.GenericNameInfo.GenericName = this.txtname.Text.Trim();
							this.GenericNameInfo.Description = this.txtDescription.Text.Trim();
							this.GenericNameSP.GenericNameEdit(this.GenericNameInfo);
							this.strGroupIdForOtherForms = this.GenericNameInfo.GenericNameId;
					...
					{
						MessageBox.Show("Enter generic name", "Pharmacy", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
						this.txtname.Select();
					}
				}
			}
		...
		private void txtDescription_KeyPress(object sender, KeyPressEventArgs e)
		{
			try
			{
				...
				{
					frmGenericNames frmGenericName = this;
					frmGenericName.intFocus = frmGenericName.intFocus + 1;
					if (this.intFocus == 2)
					{
						this.intFocus = 0;
						this.btnSave.Focus();
